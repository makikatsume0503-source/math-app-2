...
export const SimpleMathGame: React.FC<SimpleMathGameProps> = ({ mode, level, onBack, onCorrectAnswer }) => {
    ...
    function generateProblem(currentMode: GameMode, currentLevel: 1 | 2 | 3) {
        if (currentMode === 'addition') {
            if (currentLevel === 1) {
                ...
                const b = Math.floor(Math.random() * (11 - a)); // Ensures a+b <= 10
                return { a, b, ans: a + b, operator: '+', level: 1 };
            } else if (currentLevel === 2) {
                // Level 2: Sum 11-20 (Carrying usually implied, but here just range)
                // To ensure sum is 11-20
                ...
                // Level 3: Sum 21-50
                let a = Math.floor(Math.random() * 41); // 0-40
                let b = Math.floor(Math.random() * 41); // 0-40
                while (a + b <= 20 || a + b > 50) {
            ...
            // Subtraction
            if (currentLevel === 1) {
                // Level 1: No Borrowing (Simple)
                // e.g. 9-3, 15-4 (where ones digit of a >= b)
                // User requirement: "No borrowing (9-3)" and "Simple borrowing"
                // Let's interpret Level 1 as "Minuend up to 10" or "No borrow within 20"
                // Request said: "①繰り下がりなしの計算（9-3）"
                ...
                // b = y
                // Borrowing means: ones digit of a < b
                let a: number;
                let b: number;
                let aOnes: number;
                let minB: number;
                ...
                do {
                    ...
                    if (a === 10) a = 11; // Ensure 'a' is at least 11 for more borrowing scenarios

                    aOnes = a % 10;
                    minB = aOnes + 1; // b must be greater than a's ones digit for borrowing
                    maxB = 9; // b is a single digit
                } while (minB > maxB || a - minB < 0); // Regenerate if no valid 'b' for borrowing or result is negative
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 bg-white/90 backdrop-blur-md rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-[6px] border-app-blue/30 relative overflow-hidden">
            ...
            {/* Visuals - Always Show for MVP, adapted for large numbers */}
            <AnimatePresence>
                {showHint && (
                    <motion.div
                                    ...
                                    {problem.b > 10 ? (
                                        <div className="flex flex-wrap gap-1 max-w-[150px] justify-center bg-white p-2 rounded-xl border border-slate-100">
                                            {Array.from({ length: problem.b }).map((_, i) => (
                                                <div key={i} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center bg-blue-50 rounded-full border border-blue-100">
                                                    <AppleIcon size={20} className="text-blue-500" fill="currentColor" />
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
